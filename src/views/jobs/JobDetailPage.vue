<template>
  <!-- Render JobNotFoundPage component if job is not found -->
  <JobNotFoundPage v-if="jobNotFound" />
  
  <!-- Render job details if job is found -->
  <div v-else :dir="$i18n.locale === 'ar' ? 'rtl' : 'ltr'">
       ...
       <section class="bg-primary text-white py-16 md:py-20">
        <div class="container mx-auto px-4">
          <div class="max-w-4xl mx-auto">
            ...
            <!-- Add HashTags for department and location -->
            <div class="flex flex-wrap gap-2 mb-8">
              <HashTag v-if="job?.department" :tag="job.department" size="sm" />
              <HashTag v-if="job?.location" :tag="job.location.split(',')[0]" size="sm" />
            </div>
          ...
          <div class="grid grid-cols-1 lg:grid-cols-3 gap-12 max-w-6xl mx-auto">
            ...
            <div class="lg:col-span-2">
              <div class="prose prose-lg max-w-none dark:prose-invert">
                ...
                <ul>
                  <li v-for="(responsibility, index) in job?.responsibilities" :key="`responsibility-${index}`">
                    {{ responsibility }}
                  </li>
                </ul>
                ...
                <ul>
                  <li v-for="(requirement, index) in job?.requirements" :key="`requirement-${index}`">
                    {{ requirement }}
                  </li>
                </ul>
                ...
                <ul>
                  <li v-for="(benefit, index) in job?.benefits" :key="`benefit-${index}`">
                    {{ benefit }}
                  </li>
                </ul>
                ...
                <div class="p-6 rounded-lg bg-primary/5 border border-primary/20">
                  <div class="flex flex-col md:flex-row items-center justify-between gap-6">
                    ...
                    <div class="flex flex-col sm:flex-row gap-4">
                      ...
                      <Button 
                        as="RouterLink"
                        :to="`/jobs/${job?.id || 'not-found'} /apply`"
                        variant="outline"
                        class="px-6 py-3 flex items-center justify-center gap-2"
                        size="lg"
                      >
    ...
    <!-- SEO Metadata -->
    <Head>
      <title>{{ job.title }} | {{ $t('jobs.title') }} | LEAP PM</title>
      <meta name="description" :content="job.description.substring(0, 160)" />
      <meta name="robots" content="index, follow" />
      <link rel="canonical" :href="`${siteUrl}/jobs/${job.id}`" />
    </Head>
  </div>
</template>
...
import JobNotFoundPage from '@/views/errors/detail/JobNotFoundPage.vue'
...
const job = computed(() => {
  const found = jobsStore.getJobById(jobId)
...
// Job not found state
const jobNotFound = computed(() => !job.value)
...
async function validateOnChange(field: 'email' | 'phone' | 'firstName' | 'lastName' | 'resume' | 'coverLetter' | 'consentToPolicy', value: any) {
