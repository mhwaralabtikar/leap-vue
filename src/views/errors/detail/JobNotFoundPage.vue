<template>
  <div :dir="$i18n.locale === 'ar' ? 'rtl' : 'ltr'">
    <!-- SEO MetaTags -->
    <Head>
      <title>{{ $t('errors.detailPages.job.title') }} | LEAP PM</title>
      <meta name="description" :content="$t('errors.detailPages.job.description')" />
      <meta name="robots" content="noindex, follow" />
      <link rel="canonical" :href="`${siteUrl}/jobs/not-found`" />
    </Head>
    ...
    <section class="py-20 md:py-32 flex items-center min-h-screen">
      <div class="container mx-auto px-4">
        <div class="max-w-md mx-auto text-center">
          ...
          <!-- Error message -->
          <h1 class="mt-8 text-2xl md:text-3xl font-bold">{{ $t('errors.detailPages.job.heading') }}</h1>
          <p class="mt-4 text-muted-foreground">{{ $t('errors.detailPages.job.message') }}</p>
          ...
          <div class="mt-10 flex flex-col md:flex-row gap-4 justify-center">
            ...
            >
              ...
              {{ $t('jobs.viewAllJobs') || $t('jobs.title') }}
            </Button>
